...
$content = [
  ...
  [
    ...
    [
      [
        "titlelist" => "La società consociata Google che offre i servizi, tratta le tue informazioni ed è responsabile del rispetto delle leggi sulla privacy vigenti. Generalmente Google offre i propri servizi per i consumatori tramite una delle due società seguenti:",
        "listItem" =>
        [
          [
      ...
      [
        "titlelist" => "La versione dei termini che regola il nostro rapporto, che può variare in base alle leggi locali.",
        "listItem" => [],
      ],
    ],
  ...
  [
    ...
    "text" => [
      ...
      [
        "paragrafo" => "I viaggi frequenti solitamente non influiscono sul paese associato al tuo account. Se ti trasferisci in un altro paese, potrebbe occorrere circa un anno per aggiornare l'associazione del paese.",
      ],
      [
...
<body>
  <div class="container">
    <?php foreach ($content as $section) { ?>
      ...
      <?php echo "<" . $section["tag"] . ">" .  $section["title"] . "</" . $section["tag"] . ">" ?>
      <?php foreach ($section["text"] as $contenuto) { ?>

        <p> <?php echo  $contenuto["paragrafo"] ?> </p>
        
      <?php }?>


    <?php if ($ceckList) { ?>
          <?php foreach ($section["paragrafoList"] as $list) { ?>
            <ol> <?php echo $list["title"] ?> </ol>
          <?php } ?>
